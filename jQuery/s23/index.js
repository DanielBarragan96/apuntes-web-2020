...
//     // })
//     let drink = $(element);
//     let url = `${drinksAPI}/${drink.data('id')}?rev=${drink.data('rev')}`;
//     $.ajax({
//         method:"DELETE",
//         url:url
...
//     })
// }
function removeItem(id,rev){
    let url = `${drinksAPI}/${id}?rev=${rev}`;
    $.ajax({
        method:"DELETE",
        url:url
    }).done(function(result){
...
function add_drink(element){
    ...
    let drink = {
        name : $("#addDrink_name").val(),
        price: $("#addDrink_price").val(),
        image : $("#addDrink_image").val(),
        type: $("#addDrink_type").val()
    }
    $.post(drinksAPI,drink,function(data){
        console.log('adding drink sucess..');
        $("#modal_addDrink").modal("hide");
        getData();
    });
    ...
    let html =  `<li class="media">
                ...
                <div class="media-body">
                    ...
                    <ul>
                        ...
                        <li><button  type="button" class="btn btn-danger btn-sm btn-delete-drink" data-price="${drink.price}" data-id="${drink.id}" data-rev="${drink.rev}" >
            ...
            // console.log(html); onclick="remove_item(this)"
    return html;
...
function getData(){
    ...
    // })
    $.getJSON(drinksAPI,function(data,status,xhr){
        $('#drinksList').html(data.map(drinkToHTML).join(''));
        //agregar eventos a la lista
        $(".btn-delete-drink").click(function(e){
            // console.log('hola...');
            // console.log($(e.currentTarget).data('price'));
            // remove_item(e.currentTarger);
            removeItem($(e.currentTarget).data('id'),$(e.currentTarget).data('rev'))
        })
        $("#drinksList li.media:even").css('background-color','lightgreen');
        console.log($("#drinksList li.media:even").last());
        $("#drinksList li.media:even").last().css('background-color','lightcoral');
        console.log($("#drinksList li.media:even").last());
    });
}
...
//     console.log(getDrinksBtn);
//     getDrinksBtn.addEventListener('click', (event)=>{
//         getData();
//     });
// });
$(document).ready(function(){
    $('#getDrinksBtn').click(function(){
        getData();
        console.log('done from jquery');
    });
});


//https://media-cdn.tripadvisor.com/media/photo-s/14/e4/9d/84/pisco-sour-de-maracuya.jpg